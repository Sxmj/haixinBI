...
export default {
  ...
  methods: {
   ...
   async dataFormSubmit() {
      // let response=await reqRankCondition()
      // console.log(response)
      var params = {"username":this.dataForm.userName,"password":this.dataForm.password}
      params.nonce_str = common.nonce_str()
      params.sign = common.exchangeMD5(params)
      const response= await reqLogin(params)
      console.log(response)
      if(response.code==200){
        this.$router.replace({ name: "rankcondition" });
        var token=response.data.admin_token
        // console.log(token)
...
.site-wrapper.site-page--login {
  ...
  &:before {
    ...
    height: 100%;
    content: "";
    background: url(~@/assets/img/BI_login_imgbg.png) no-repeat center;
